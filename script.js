...
let isAllCompleted = false; // todos list status
...
// check the todos list to see if completed(checked) or not (isCompleted)
...
const onClickCompleteAll = () => {
    if(!getAllTodos().length) return; // if todos list length is 0, return;

    if(isAllCompleted) incompleteAll(); // if isAllCompleted is true then make sure everything in todos is marked complete
    else completeAll(); // if isAllCompleted is false then finish processing todos
    setIsAllCompleted(!isAllCompleted); // isAllCompleted toggle
    paintTodos(); // render the new todos
    setLeftItems()
}

const appendTodos = (text) => {
    const newId = id + 1; // create a new id to link with the new todo list item giving the new item a new id
    setId(newId)
